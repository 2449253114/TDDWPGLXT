const caihongPayConfig = require('../../../common/config/caihong-pay-config');
const {
...
async function callPayApi(params) {
	try {
		...
		const result = await uniCloud.request({
			url: `${caihongPayConfig.api_url}/mapi.php`,
			method: "POST",
			data: params,
			header: {
		...
		throw new Error(`彩虹易支付API请求失败: ${error.message}`);
	}
}
...
async function createOrder() {
	...
	// 2. 生成签名
	params.sign = createQixiangPayMD5Sign(params, mch_key);
...
module.exports = {
	caihongPayCreateOrder: createOrder
}