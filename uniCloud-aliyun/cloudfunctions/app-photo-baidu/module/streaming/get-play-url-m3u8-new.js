...
async function requestM3U8PlayUrl(headers, fsid, album_id, tid) {
	...
	//throw new Error('视频迁移中，预计8点左右完成');
	...
	// TODO 2024-0721起   改成了一小时内最多可以请求135个视频（5个子账号加起来就这么多，为了减少请求次数压力，避免高频请求导致封号），否则提示换其他的视频看或晚点儿再来看这个视频
	let hourlyResult = await hourlyRequestCountCollection.get()
	let hourlyData = hourlyResult.data; 
	// 检查当前时段的请求次数是否达到每小时120次的上限。
	if (checkHourlyRequestLimit(hourlyData)) {
		// 如果达到上限
		//throw new Error('');// 这里你帮我写，大概就是告诉用户换其他的视频看或晚点儿再来看这个视频，意思是这个视频现在观看的人较多，让他去看别的或者晚点儿再看
		throw new Error('当前视频观看人数较多，请观看其他视频或晚点再来。');
	}
	// 后期增加如果hourlyData.length=0，则先新建24个数据，目前有yike-hourly-request-count.init_data.json直接生成
	// 假设这是一次请求
	await updateHourlyRequestCount(hourlyData);
	
	
	// TODO 2024-0721起   改成了多个账号Cookie随机请求
	
	// 检查 m3u8Record 是否存在以及是否有 sub_account_cookie_ids 属性
	let usedCookieIds = (m3u8Record && m3u8Record.sub_account_cookie_ids) || []
	
	// 先判断是否已经记录了5个不同的 Cookie ID，如果是，则清空集合
	...
	const newCookieId = randomCookieObj.id
...
/**
 * 检查当前时段的请求次数是否达到每小时135次的上限。
 * 
 * @param {Array} hourlyData - 包含所有时段请求次数的数组数据。
 * @returns {boolean} - 如果请求次数达到上限，返回true；否则，返回false。
 */
function checkHourlyRequestLimit(hourlyData) {
	// 获取当前时间
	const now = new Date(Date.now());
	const currentHour = now.getHours();

	// 计算当前时间对应的时间段_id
	let _id = `hourly_slot_${currentHour}`;

	// 查找当前时段的索引
	let index = hourlyData.findIndex(item => item._id === _id);

	// 检查请求次数是否达到上限
	if (hourlyData[index].request_count >= 135) {
		console.log('已经达到每小时120次的请求限制！');
		// 这里可以添加额外的逻辑，比如抛出错误或返回特定信息
		return true; // 达到上限，返回true
	} else {
		console.log('请求次数未达到上限，继续处理...');
		return false; // 未达到上限，返回false
	}
}
