...
// 定义请求播放URL的函数
async function requestMP4PlayUrl(headers, album_id, uk, tid, fsid) {
	// 发送请求并设置不自动跟随重定向
	const apiUrl =
...
module.exports = async function() {
	...
	if (hasPurchasedAlbum || hasPurchasedFile) {
		try {
			const playUrl = await requestMP4PlayUrl(headers, album_id, uk, tid, fsid);
			// 构造返回数据
			const data = {
				playUrl,
				Cookie: headers.Cookie,
