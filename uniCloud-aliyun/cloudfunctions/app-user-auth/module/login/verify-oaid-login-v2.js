...
module.exports = async function() {
	...
	// 检查 device_oaid 是否有效
	if (!device_oaid || typeof device_oaid !== 'string' || device_oaid.trim() === "") {
		// 修复某些手机传递的body提取不到device_oaid的值
		// 使用正则表达式从 httpInfo.body 中提取 device_oaid，因为httpInfo.body没有经过处理
		const deviceOaidRegex = /"device_oaid"\s*:\s*"([^"]+)"/;
		const deviceOaidMatch = httpInfo.body.match(deviceOaidRegex);
		
		if (deviceOaidMatch && deviceOaidMatch[1]) {
			device_oaid = deviceOaidMatch[1];
			//return createResponse(STATE_CODE.SUCCESS, `请求数据: device_oaid == ${device_oaid}`);
		} else {
			return createResponse(STATE_CODE.FAIL, "无效的请求数据: device_oaid 缺失或为空");
		}
	}
	...
	if (queryUserResult['affectedDocs'] == 1) { // 有记录，此账号存在
		...
		let registerUserResult = await userCollection.add({ // 注册 - 新增用户
			...
			vip: true,// 注册赠送，2024-0106-起赠送1天VIP
		...
		sendSystemNotice(appSystemNoticeCollection,{
			...
			content: "欢迎加入我们！请注意，自2024年5月17日18:00起，我们调整了新用户的福利。注册当天，您可以享受1天VIP会员体验（如果不是会员，请重启APP即可解决），之后每周一至周三将继续享有1次免费观看的权益。我们已暂停了注册赠送7天VIP会员试用的活动。祝您在平台上的每一刻都充满乐趣！"
		});
