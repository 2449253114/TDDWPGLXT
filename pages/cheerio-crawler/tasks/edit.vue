  ...
  <view class="uni-container">
    <uni-forms ref="form" :model="formData" validateTrigger="bind"  label-width="220px">
      ...
      <uni-forms-item name="urls" label="采集地址" required>
        <uni-easyinput placeholder="存储要采集的地址，一行一个URL" :value="urlDecodeChange(formData.urls)" @input="updateUrls" trim="end" :auto-height="false" type="textarea" :maxlength="-1"></uni-easyinput>
      </uni-forms-item>
  ...
  export default {
    ...
    methods: {
      ...
      getDetail(id) {
        ...
        db.collection(dbCollectionName).doc(id).field("task_title,urls,source_url_selector,title_selector,nickname_selector,your_story_selector,your_story_img_selector,your_story_src_prefix,your_story_description_selector").get().then((res) => {
