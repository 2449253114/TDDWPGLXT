	...
	<view class="uni-container">
		<view class="header">
			...
			<view class="right">
				...
				<button type="default" class="uni-button" style="width: 100px;margin-left: 20rpx;"
					@click="startTasks()">开始采集</button>
					
				<button type="warn" class="uni-button" style="width: 100px;margin-left: 20rpx;"
					@click="testChtml()">测试采集</button>	
		...
		<view class="proview-textarea-box">
			<view class="t-h2">
				采集地址
			</view>
			<uni-easyinput placeholder="存储要采集的地址，一行一个URL" :value="urlDecodeChange(task.config.urls)" @input="updateUrls"
				trim="end" :auto-height="false" type="textarea" :maxlength="-1" />
		</view>
			...
			<view class="uni-group">
				...
				<view class="uni-sub-title">
					<view class="wrap">
						<text>采集地址：</text>
						<uni-easyinput
							:value="`${ task.page.current !== 0 ? task.urlsList[task.page.current - 1].url : '等待中'}`"
							:disabled="true" placeholder="等待中"></uni-easyinput>
					</view>
				</view>
				<view class="uni-sub-title m-l-10" v-if="task.urlsList.length !== 0">
					...
					<text> |
						同步：{{ taskStateText(task.urlsList[currentPage(task.page.current - 1)].custom_info.state.sync) }}</text>
				</view>
			</view>
		</view>
...
<script>
	...
	import {
		collectData
	} from '@/pages/cheerio-crawler/tasks/ubearcn/ubearcn.js'
	
	import {
		concatenatedUrls
	} from '@/pages/cheerio-crawler/tasks/ubearcn/a-href.js'
	...
	export default {
		...
		methods: {
			testChtml() {
				...
				uni.downloadFile({
					url: 'http://www.ubearcn.com/bc/20200508w/bmwe/01.jpg', //仅为示例，并非真实的资源
					success: (res) => {
						console.log(res)
			 ...
			 * 初始化采集任务配置数据
			 */
			async initTaskConfig() {
				const task_id = this.task.config._id
				this.task.config.urls = concatenatedUrls
				let urls = toUrlsList(concatenatedUrls)
			...
			async checkExistingUrls(task_id, urls) { // 检查现有的URL链接在云端是否有记录，有记录则说明已采集过了，反之未采集
				...
				const {
					filteredUrlsList,
					filteredCount
				} = filterExistingItems(this.task.urlsList, data);
				// 打印过滤了多少个项目的信息
			...
			async fetchHtmlContent() { // 获取Html内容
				...
				return new Promise((resolve, reject) => {
					http({
						...
						const html = res.data
						// 采集Html数据
						let result = collectData(html, BASE_URL);
						// 将采集地址也添加到来源地址数组中
			...
			async syncToCloud(item) {
				...
				await handleUploadFiles(item.custom_info.uploadPictureParam, (index, progress) => {
					...
					.then(updatedList => {
						...
						for (let i = 0; i < updatedList.length; i++) {
							// 当前页采集数据
							this.task.urlsList[this.task.page.current - 1].custom_info.formData.your_story[i]
								.thumburl[1] = updatedList[i].url;
							if (i == 0) {
								// 设置漫画文封面
								this.task.urlsList[this.task.page.current - 1].custom_info.formData.thumburl[1] =
									updatedList[i].url;
							}
						}
				...
				try {
					const {
						formData
					} = this.task.urlsList[this.task.page.current - 1].custom_info
