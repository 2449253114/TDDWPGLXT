<template>
	<view class="container">
					...
					<view v-else>
						<view class="section-title-container">
							...
							<view class="status-summary">
								...
								<text class="status-count abnormal">异常中: {{abnormalUserCount}}</text>
							...
							<view class="section-title-container">
								...
								<view class="status-filter">
									...
									<view class="radio-group">
										...
										<!-- 异常中用于统计短期邀请异常阈值（3-5天内邀请50人，这明显不正常） -->
										<view class="radio-item" @click="filterByStatus('abnormal')">
											<view :class="['radio-circle', {'selected': statusFilter === 'abnormal'}]"></view>
											<text>异常中</text>
										</view>
		...
		<view v-if="ipSummary.length > 0" class="ip-statistics">
			...
			<view class="ip-list">
				<view v-for="(item, index) in filteredIpSummary" :key="index"
	...
	export default {
		data() {
			return {
				...
				filteredIpSummary: [], // 新增：当前邀请人的IP统计
				checkedInviteCodes: new Set(), // 新增：用于去重的已检测邀请码集合
				...
				highRiskUserCount: 0,
				abnormalUserCount: 0, // 新增：异常中状态用户数量
				ABNORMAL_THRESHOLD: 50, // 新增：短期邀请异常阈值（3-5天内邀请50人）
		...
		methods: {
			...
			/**
			 * 检测短期内大量邀请（3-5天内邀请50人以上）
			 */
			async checkShortTermInvites() {
				// 如果没有邀请用户数据，直接返回
				if (!this.invitedUsers || this.invitedUsers.length === 0) {
					return;
				}
				...
				// 获取当前时间
				const now = new Date();
				const fiveDaysAgo = new Date(now.getTime() - 5 * 24 * 60 * 60 * 1000); // 5天前

				// 筛选3-5天内的邀请记录
				const recentInvites = this.invitedUsers.filter(user => {
					const inviteDate = new Date(user.invite_time);
					return inviteDate >= fiveDaysAgo && inviteDate <= now;
				});

				// 如果3-5天内邀请人数超过阈值，记录为异常
				if (recentInvites.length >= this.ABNORMAL_THRESHOLD) {
					this.checkedUsers[this.myInviteCode] = {
						...this.checkedUsers[this.myInviteCode],
						status: '异常中',
						shortTermInviteCount: recentInvites.length,
						checkTime: Date.now()
					};
			...
			countUsersByStatus() {
				...
				this.permanentVipUsers.forEach(user => {
					...
					} else if (user.status === '异常中') {
						this.abnormalUserCount++;
			...
			async queryPermanentVipUsers() {
				...
				try {
					...
					// 更新任务状态
					this.taskStatus.message = `找到${total}个永久会员，正在加载数据...`
					this.taskStatus.progress = 10
					...
					for (let currentPage = 1; currentPage <= totalPages; currentPage++) {
						...
						// 更新任务状态
						this.taskStatus.message = `正在查询第${currentPage}/${totalPages}页...`;
						this.taskStatus.progress = Math.round((currentPage / totalPages) * 50);
					...
					// 更新任务状态
					this.taskStatus.message = '正在统计用户状态...'
					this.taskStatus.progress = 85

					// 统计不同状态的用户数量
					this.countUsersByStatus()

					// 查询完成后显示结果
					this.taskStatus.message = '查询完成';
					this.taskStatus.progress = 100;
					if (total > 0) {
						uni.showToast({
					...
					// 延时关闭任务状态显示
					setTimeout(() => {
						this.taskStatus.running = false
					}, 2000)
				} catch (error) {
					console.error('查询永久会员出错:', error)

					// 更新任务状态
					this.taskStatus.message = '查询失败: ' + error.message
					// 延时关闭任务状态显示
					setTimeout(() => {
						this.taskStatus.running = false
					}, 2000)
			...
			async checkViolation() {
				...
				try {
					...
					// 5. 检测短期内大量邀请情况
					await this.checkShortTermInvites();
					...
					// 更新任务状态
					this.taskStatus.message = '检测失败: ' + error.message
			...
			updateGlobalIpStats() {
				...
				try {
					...
					// 检查当前邀请码是否已处理过，用于去重
					if (this.checkedInviteCodes.has(this.myInviteCode)) {
						// 如果已处理过，先移除该邀请码相关的统计
						for (const ip in globalIpMap) {
							globalIpMap[ip].inviters = globalIpMap[ip].inviters.filter(
								inv => inv.code !== this.myInviteCode
							);
							// 如果没有关联的邀请人，重新计算总数和风险等级
							if (globalIpMap[ip].inviters.length === 0) {
								globalIpMap[ip].count = 0;
								globalIpMap[ip].riskLevel = '正常';
							} else {
								globalIpMap[ip].count = globalIpMap[ip].inviters.reduce(
									(sum, inv) => sum + inv.invitedCount, 0
								);
								globalIpMap[ip].riskLevel = globalIpMap[ip].count >= this.HIGH_RISK_THRESHOLD ?
									'高风险' :
									globalIpMap[ip].count >= this.MEDIUM_RISK_THRESHOLD ?
									'中风险' :
									'正常';
							}
						}
					...
					for (const ipItem of this.ipSummary) {
						...
						if (globalIpMap[ip]) {
							// 添加或更新邀请人信息
							const inviterIndex = globalIpMap[ip].inviters.findIndex(
								inv => inv.code === this.myInviteCode
							);
							if (inviterIndex !== -1) {
								globalIpMap[ip].inviters[inviterIndex].invitedCount = count;
							} else {
					...
					// 更新全局IP统计
					this.globalIpStats = Object.values(globalIpMap).filter(
						item => item.count > 0 || item.isInviterIp
					);
					...
					// 记录当前邀请码已处理
					this.checkedInviteCodes.add(this.myInviteCode);
				} catch (error) {
					console.error('更新全局IP风险统计出错:', error);
				}
			},
			...
			async processBatchCheck(users) {
				...
				for (const user of users) {
					...
					// 更新任务状态
					this.taskStatus.message = `检测进度: ${currentIndex}/${totalUsers}`
					this.taskStatus.progress = Math.floor((currentIndex / totalUsers) * 100)
					this.taskStatus.current = currentIndex;
				...
				// 更新任务状态
				this.taskStatus.message = '批量检测完成';
				this.taskStatus.progress = 100;
				setTimeout(() => {
					this.taskStatus.running = false;
				}, 1000);
				...
				uni.showModal({
					...
					success: async (res) => {
						if (res.confirm) {
							...
							this.taskStatus = {
								running: true,
								message: `正在封禁${highRiskUsers.length}个高风险用户...`,
								progress: 0,
			...
			async processBanUsers(users) {
				...
				try {
					for (const user of users) {
						...
						// 更新任务状态
						this.taskStatus.message = `正在封禁第${processed}/${total}个用户...`;
						this.taskStatus.progress = Math.round((processed / total) * 100);
						this.taskStatus.current = processed;
					...
					// 完成任务
					this.taskStatus.message = `成功封禁${successCount}个高风险用户`;
					this.taskStatus.progress = 100;
					setTimeout(() => {
						this.taskStatus.running = false;
					}, 1000);
					...
					this.taskStatus.message = '批量封禁失败: ' + error.message;
					setTimeout(() => {
						this.taskStatus.running = false;
					}, 2000);
			...
			saveDetectionResult() {
				...
				let shortTermInviteCount = 0; // 短期邀请计数
				...
				this.checkedUsers[this.myInviteCode] = {
					...
					shortTermInviteCount: shortTermInviteCount,
					checkTime: Date.now()
				}
				...
				if (ipExceedBanThreshold) {
					...
					uni.showModal({
						title: '异常行为警告',
						content: `检测到该用户在3-5天内邀请了${shortTermInviteCount}个账号，行为异常，已标记为异常中！`,
						showCancel: false
					});
				}
			...
			async queryInvitedUsers() {
				...
				try {
					...
					// 只保留数量大于等于中风险阈值的IP地址
					this.ipSummary = ipArray.filter(item => item.count >= this.MEDIUM_RISK_THRESHOLD);

					// 检查是否存在高风险IP（同IP注册 >= 高风险阈值）
					this.hasHighRiskIP = this.ipSummary.some(item => item.count >= this.HIGH_RISK_THRESHOLD);
			 ...
			 * 计算当前邀请人的IP统计
			 */
			updateFilteredIpSummary() {
				// 确保只显示当前邀请人的IP统计
				this.filteredIpSummary = this.ipSummary.filter(item =>
					this.invitedUsers.some(user => user.login_ip === item.ip)
				);
			},
...
<style lang="scss">
	...
	.status-count.abnormal {
		background-color: #e6f7ff;
		color: #1890ff;
	}

	.invite-code-item.status-abnormal {
		background-color: #e6f7ff;
		border-color: #91d5ff;
	}

	.invite-code-item.status-abnormal.selected {
		background-color: #1890ff;
		color: white;
