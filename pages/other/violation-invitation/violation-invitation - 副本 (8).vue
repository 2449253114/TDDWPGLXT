				...
				<view class="global-ip-section">
					...
					<view v-else>
						<view class="section-title-container">
							<text class="section-title">高风险IP统计 (共{{globalIpStats.length}}个)</text>
							<button type="primary" size="mini" @click="exportIpBlacklist">导出IP黑名单</button>
						</view>
						
						<view v-if="globalIpStats.length > 0" class="ip-stats-scroll-container">
							<view class="ip-stats-list">
								<view 
									v-for="(item, index) in globalIpStats" 
									:key="index" 
									:class="['ip-stats-item', {'high-risk': item.count >= HIGH_RISK_THRESHOLD, 'medium-risk': item.count >= MEDIUM_RISK_THRESHOLD && item.count < HIGH_RISK_THRESHOLD}]"
								>
									...
									<view class="ip-stats-inviters">
										<text class="inviters-label">邀请人({{item.inviters.length}}个):</text>
										<view class="inviters-list">
											<view 
												v-for="(inviter, idx) in item.inviters" 
												:key="idx"
												class="inviter-code"
												@click="selectInviteCode(inviter.inviteCode)"
											>
												{{inviter.inviteCode}} ({{inviter.count}}人)
											</view>
										</view>
									</view>
								</view>
					...
					<view v-else>
						...
						<view v-if="filteredPermanentVipUsers.length > 0" class="invite-code-scroll-container">
							<view class="invite-code-flow-list">
								<view 
									v-for="(item, index) in filteredPermanentVipUsers" 
									:key="index" 
		...
		<uni-popup ref="ipBlacklistPopup" type="center">
			<view class="popup-content">
				<view class="popup-title">
					<text>IP黑名单</text>
					<view class="popup-close" @click="closeIpBlacklistPopup">×</view>
				</view>
				<view class="popup-body">
					<textarea class="ip-blacklist-textarea" v-model="ipBlacklistText" readonly></textarea>
					<button type="primary" size="mini" @click="copyIpBlacklist">复制到剪贴板</button>
				</view>
			</view>
		</uni-popup>
	</view>
...
<script>
	...
	const GLOBAL_IP_STATS_KEY = 'global_ip_stats'
		...
		methods: {
			...
			applyStatusFilter() {
				if (this.statusFilter === 'all') {
					this.filteredPermanentVipUsers = [...this.permanentVipUsers]
					return
				}
				
				this.filteredPermanentVipUsers = this.permanentVipUsers.filter(user => {
					switch (this.statusFilter) {
						case 'normal':
							return !user.status || user.status === '正常'
						case 'medium':
							return user.status === '中风险'
						case 'high':
							return user.status === '高风险' || user.status === '已封禁'
						default:
							return true
					}
				})
			...
			async batchCheckViolation() {
				...
				uni.showModal({
					...
					success: async (res) => {
						...
						uni.showModal({
							title: '批量检测确认',
							content: `将对${usersToCheck.length}个用户进行批量检测，可能需要较长时间，是否继续？`,
							success: async (res) => {
								if (res.confirm) {
