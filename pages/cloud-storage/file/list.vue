<template>
	<view>
        ...
        <button class="uni-button" type="default" size="mini" @click="search">搜索</button> -->
			...
			<view class="uni-group">
				<!-- 将大于1秒且小于n秒的视频状态设为审核中 -->
				<input class="uni-search" type="text" v-model="videoTime" placeholder="请输入秒数，并将时长小于n秒的视频设为审核中" />
				<button class="uni-button" type="warn" @click="updateVideoStatus">设为审核中</button>
				<!-- 批量修改状态为审核中 -->
				<button class="uni-button" type="default" :disabled="selectedIndexs.length == 0" @click="changeStatus(2)">批量修改状态为审核中</button>
			</view>
		</view>
		<view class="uni-container">
			<unicloud-db ref="udb" :collection="collectionList"
				field="album_id,fsid,category,ctime,bytes,duration_format,thumburl,status,extra_info,duration_ms_long,md5"
				...
				<uni-table ref="table" :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe
					...
					<uni-tr>
						...
						<uni-th align="center" filter-type="range" @filter-change="filterChange($event, 'duration_ms_long')" sortable @sort-change="sortChange($event, 'duration_ms_long')">总时长毫秒</uni-th>
						...
						<uni-th align="center" filter-type="select"
							...
							sortable @sort-change="sortChange($event, 'status')">发布状态</uni-th>
						<uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'md5')">MD5</uni-th>	
						<uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'album_id')">相册ID</uni-th>	
						<uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'fsid')">文件ID</uni-th>	
					...
					<uni-tr v-for="(item,index) in data" :key="index">
						...
						<uni-td align="center">{{item.duration_ms_long}}</uni-td>
		...
		methods: {
			...
			async changeStatus(status = 2) {
				...
				if (updateRes.updated > 0) {
					uni.showToast({
						title: `成功修改 ${updateRes.updated} 条记录的状态为审核中。`
					});
					this.$refs.table.clearSelection()
					this.$nextTick(() => {
						this.$refs.udb.loadData()
			...
			async updateVideoStatus() {
				...
				// 查询符合条件的视频
				const { result: updateRes } = await fileCollection.where({
					duration_ms_long: dbCmd.gte(1).and(dbCmd.lte(n * 1000)),
					status: 1, 
					category: 1, // 1=视频 、3=图片
				}).update({
					status: 2
				})
				...
				if (updateRes.updated > 0) {
					uni.showToast({
						title: `成功将 ${updateRes.updated} 条视频状态设为审核中。`
					});
					this.$refs.table.clearSelection()
					this.$nextTick(() => {
						this.$refs.udb.loadData()
			...
			filterChange(e, name) {
				...
				console.log('this._filter[name]', this._filter[name])
