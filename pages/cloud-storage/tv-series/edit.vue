...
<script>
	...
	export default {
		...
		methods: {
			 ...
			 */
			submitForm(value) {
				// 使用对象解构来从formData中提取所有字段，同时排除_id字段
				// _id字段被单独提取并且不会被使用，...valueWithoutId包含了剩余的字段
				const { _id, ...valueWithoutId } = this.formData;
			...
			async getDetail(id) {
				...
				db.collection(dbCollectionName).doc(id).field("album_id,show_name,episodes,description,status").get().then(
					async (res) => {
						const data = res.result.data[0]
						if (data) {
							// 先只赋值album_id
							this.formData.album_id = data.album_id
							// 加载相册文件
							await this.loadData()
							// 最后更新视图
							this.formData = data
			...
			async addEpisodeDialogToggle() {
				...
				this.updetaAvailableCoverFiles()
				this.updetaAvailableVideoFiles()
