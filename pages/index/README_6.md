...
1. app下载链接
2. app二维码链接‘
3. 复制链接拷贝的文本：好友邀请你加入相册《照片秒传相册》\n" +
                                "从下方链接加入相册，复制共享口令(IzaawNfSb)后打开【一刻相册】App，即可找到 *^共享^*\n" +
                                "点击查看全部照片：https://photo.baidu.com/photo/wap/albumShare/invite/IzaawNfSb\n" +
                                "-----------------------------\n" +
                                "应用商店搜索：一刻相册
搜索页还差
1. 暗号搜索
...
我需要你集合我下面的代码，来实现在用户请求视频URL时检查其IP地址与登录时的IP地址是否一致，可以将检查IP是否一致性封装为调用方法，不一致就提示，否则正常走
```javascript
const {
	userCollection,
