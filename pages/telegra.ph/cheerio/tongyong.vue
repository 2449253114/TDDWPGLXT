<template>
	<view>
		<textarea :value="html2" :maxlength="-1" />
		<button @click="startHtml()">开始采集</button>
	</view>
</template>
...
<script>
	...
	function collectData(html, config) {
	    ...
	    result.source_url = evaluateHref(config.source_url, $);
	...
	// 配置对象示例
	const config1 = {// 用于采集https://telegra.ph/肉欲同学会-01-02
	...
	const config2 = {// 用于采集http://darknight.party/comic
    ...
    <p>
      ...
      <a href="http://darknight.party/comic/wp-content/uploads/2023/02/265-1.jpg">
		...
		methods: {
			startHtml() {
				// 测试采集函数
				const html1 = this.html1;
				const html2 = this.html2;
